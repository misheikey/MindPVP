...
class AutoEatPlugin {
  ...
  async onLoad() {
    ...
    this.bot.autoEat.setOpts({
      ...
      minHunger: 1,                    // 饥饿值 ≤18 时开始吃
      minHealth: 10,                    // 血量 ≤12 时优先吃高饱和度食物
...
async initPlugins() {
  ...
  const AIChatPlugin = require('./plugins/AIChat');  // 假设插件文件路径
  const AIChat = new AIChatPlugin(this.bot, this.pluginManager);
  await AIChat.onLoad();
  this.pluginManager.plugins.set('AIChat', AIChat);

  const AggressiveModePlugin = require('./plugins/AggressiveMode');  
  const AggressiveMode = new AggressiveModePlugin(this.bot, this.pluginManager);
  await AggressiveMode.onLoad();
  this.pluginManager.plugins.set('AggressiveMode', AggressiveMode);
